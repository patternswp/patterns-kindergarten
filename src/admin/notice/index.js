/*CSS*/
/* global PatternsKindergartenLocalize */
...
class PatternsKindergartenSettingsApi {
	...
	bindEvents() {
		document.addEventListener( 'click', ( event ) => {
			...
			if ( target.closest( '.patterns-kindergarten-gsn-close' ) ) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				target.closest( '.patterns-kindergarten-rn-permanent-close' )
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				target.closest( '.patterns-kindergarten-rn-temporary-close' )
			) {
				this.modifySettings(
				...
				const installBtn = target.closest(
					'.patterns-kindergarten-install-plugins'
				);
				if ( installBtn ) {
					installBtn.classList.add(
						'patterns-kindergarten-processing'
					);
					this.installPlugins();
				}
			}
	...
	updateUI() {
		...
		if ( ! this.isLoadReview() ) {
			const reviewNoticeElement = document.getElementById(
				'patterns-kindergarten-review-notice'
			);
